...
class Skill(abc.ABC):
    ...
    One caveat of this approach is that a new skill object will be instantiated
    every time it is invoked, but that is a small cost compared to the time 
    saved from not having to specify those parameters anymore.
    '''
    
    def __init__(self, client, channel, thread):
