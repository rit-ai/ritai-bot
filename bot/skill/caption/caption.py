...
WORD_MAP_ID = '1bt_TmTC_rUcss2MJsG_C_6DtwEttRVKc'
WORD_MAP_NAME = 'word_map.json'
MODEL_ID = '1FYZ446OPEqhe-uLkgyVICjD_3-N3IZn1'
MODEL_NAME = 'model.pt'
...
def caption_image(img, encoder, decoder, word_map, beam_size=3):
    """
    ...
    img = numpy.array(Image.fromarray(arr).resize(img, (256, 256)))
    ...
    caption = complete_seqs[i]
    alphas = complete_seqs_alpha[i]

    return caption, alphas


def visualize_att(img, caption, alphas, rev_word_map, smooth=True):
    """
...
class SkillCaption(skill.Skill):
    ...
    def __init__(self):
        # Downloads the model to the same level as the "caption.py" file, if it 
        # isn't already present.
        dest = pathlib.Path(os.path.dirname(os.path.realpath(__file__))) / 'models'
        download_and_extract_model(dest)
        
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        # Load model
        '''
        checkpoint = torch.load(str(dest / MODEL_NAME))
        ...
        with open(str(dest / WORD_MAP_NAME), 'r') as j:
    ...
    def execute(self, prompt):
        ...
        caption, alphas = caption_image(img, self.encoder, self.decoder, self.word_map)
        alphas = torch.FloatTensor(alphas)
        
        # Visualize caption and attention of best sequence
        visualize_att(img, caption, alphas, self.rev_word_map)
